...
class Seasons {
  ...
  constructor() {
    ...
    this.currentIndex = 0;
    this.seasonArr = ["spring", "summer", "fall", "winter"];
  }
   ...
   */
  next() {
    if (this.currentIndex >= this.seasonArr.length - 1) {
      this.currentIndex = 0;
    } else {
      this.currentIndex += 1;
    }

    return this.seasonArr[this.currentIndex];
  }
}
...
class Car {
  ...
  drive(distance) {
    ...
    this.odometer += intDistance;

    let gasUsed = intDistance / this.mpg;
    if (this.tank - gasUsed >= 0) {
      this.tank -= gasUsed;
    } else {
      this.tank = 0;
    }
   ...
   */
  refuel(gallons) {
    let gasAdded = this.tank + gallons;

    if (gasAdded > this.tankSize) {
